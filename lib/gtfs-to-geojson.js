...
const getGeoJSONByFormat = async (config, routeId, directionId) => {
  const allGeo = {};
  if (config.outputFormat.contains('envelope')) {
    allGeo['envelope'] = envelope(config, routeId, directionId);
  }

  if (config.outputFormat.contains('convex')) {
    allGeo['convex'] = convex(config, routeId, directionId);
  }

  if (config.outputFormat.contains('lines-and-stops')) {
    allGeo['linesAndStops'] = linesAndStops(config, routeId, directionId);
  }

  if (config.outputFormat.contains('lines')) {
    allGeo['lines'] = lines(config, routeId, directionId);
  }

  if (config.outputFormat.contains('lines-buffer')) {
    allGeo['linesBuffer'] = linesBuffer(config, routeId, directionId);
  }

  if (config.outputFormat.contains('lines-dissolved')) {
    allGeo['linesDissolved'] = linesDissolved(config, routeId, directionId);
  }

  if (config.outputFormat.contains('stops')) {
    allGeo['stops'] = stops(config, routeId, directionId);
  }

  if (config.outputFormat.contains('stops-buffer')) {
    allGeo['stopsBuffer'] = stopsBuffer(config, routeId, directionId);
  }

  if (config.outputFormat.contains('stops-dissolved')) {
    allGeo['stopsDissolved'] = stopsDissolved(config, routeId, directionId);
  }
...
module.exports = async initialConfig => {
  ...
  for (const agency of config.agencies) {
    ...
    return await buildGeoJSON(agencyKey, config, outputStats);
  }
